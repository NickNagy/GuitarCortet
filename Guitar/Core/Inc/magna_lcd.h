/*
 * lcd.h
 *
 *  Created on: Nov 17, 2021
 *      Author: Nick Nagy
 */

#ifndef INC_MAGNA_LCD_H_
#define INC_MAGNA_LCD_H_

#include <stdlib.h>
#include <stdbool.h>
#include <string>
#include "fmc.h"

namespace magna {

/**
 * @class LCD
 * @brief virtual base class for LCD display
 */
class LCD {
protected:
	uint16_t width, height;

	virtual void setCursorPosition(uint16_t x0, uint16_t x1, uint16_t y0, uint16_t y1) = 0;
public:
	LCD(uint16_t width, uint16_t height) {
		this->width = width;
		this->height = height;
	}
	virtual ~LCD() {}

	/* virtual functions */
	virtual void fill(uint16_t color) = 0;
	virtual void drawRect(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) = 0;
	virtual void fillRect(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) = 0;
	virtual void drawPixel(uint16_t x, uint16_t y, uint16_t color) = 0;
	virtual void drawCircle(uint16_t x0, uint16_t y0, uint16_t r, uint16_t color) = 0;
	virtual void fillCircle(uint16_t x0, uint16_t y0, uint16_t r, uint16_t color) = 0;
	virtual void drawTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) = 0;
	virtual void fillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color) = 0;
	virtual void drawChar(uint16_t x, uint16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size) = 0;
	virtual void printText(std::string text, uint16_t x, uint16_t y, uint16_t color, uint16_t bg, uint8_t size) = 0;
	virtual void printImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data, uint32_t size) = 0;
	virtual void setRotation(uint8_t rotate) = 0;
	virtual void drawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) = 0;
};

/**
 * @class ILI9341
 * @brief class for drawing / writing on ILI9341 LCD display
 */
/* ILI9341 specific defines */
#define ILI9341_WIDTH       240
#define ILI9341_HEIGHT      320
#define ILI9341_PIXEL_COUNT	ILI9341_WIDTH * ILI9341_HEIGHT
//ILI9341 LCD commands
#define ILI9341_RESET			 		    	0x01
#define ILI9341_SLEEP_OUT		  			0x11
#define ILI9341_GAMMA			    			0x26
#define ILI9341_DISPLAY_OFF					0x28
#define ILI9341_DISPLAY_ON					0x29
#define ILI9341_COLUMN_ADDR					0x2A
#define ILI9341_PAGE_ADDR			  		0x2B
#define ILI9341_GRAM				    		0x2C
#define ILI9341_TEARING_OFF					0x34
#define ILI9341_TEARING_ON					0x35
#define ILI9341_DISPLAY_INVERSION		0xb4
#define ILI9341_MAC			        		0x36
#define ILI9341_PIXEL_FORMAT    		0x3A
#define ILI9341_WDB			    	  		0x51
#define ILI9341_WCD				      		0x53
#define ILI9341_RGB_INTERFACE   		0xB0
#define ILI9341_FRC					    	0xB1
#define ILI9341_BPC					    	0xB5
#define ILI9341_DFC				 	    	0xB6
#define ILI9341_Entry_Mode_Set		0xB7
#define ILI9341_POWER1						0xC0
#define ILI9341_POWER2						0xC1
#define ILI9341_VCOM1							0xC5
#define ILI9341_VCOM2							0xC7
#define ILI9341_POWERA						0xCB
#define ILI9341_POWERB						0xCF
#define ILI9341_PGAMMA						0xE0
#define ILI9341_NGAMMA						0xE1
#define ILI9341_DTCA							0xE8
#define ILI9341_DTCB							0xEA
#define ILI9341_POWER_SEQ					0xED
#define ILI9341_3GAMMA_EN					0xF2
#define ILI9341_INTERFACE					0xF6
#define ILI9341_PRC				   	  	0xF7
#define ILI9341_VERTICAL_SCROLL 	0x33
#define ILI9341_PARTIAL_MODE_ON		0x12
#define ILI9341_PARTIAL_AREA		0x30
#define ILI9341_DISPLAY_INVERSION_ON		0x21
#define ILI9341_DISPLAY_INVERSION_OFF		0x20
#define ILI9341_MEMCONTROL         0x36
#define ILI9341_MADCTL_MY  0x80
#define ILI9341_MADCTL_MX  0x40
#define ILI9341_MADCTL_MV  0x20
#define ILI9341_MADCTL_ML  0x10
#define ILI9341_MADCTL_RGB 0x00
#define ILI9341_MADCTL_BGR 0x08
#define ILI9341_MADCTL_MH  0x04
//List of colors
#define ILI9341_COLOR_BLACK           0x0000
#define ILI9341_COLOR_NAVY            0x000F
#define ILI9341_COLOR_DGREEN          0x03E0
#define ILI9341_COLOR_DCYAN           0x03EF
#define ILI9341_COLOR_MAROON          0x7800
#define ILI9341_COLOR_PURPLE          0x780F
#define ILI9341_COLOR_OLIVE           0x7BE0
#define ILI9341_COLOR_LGRAY           0xC618
#define ILI9341_COLOR_DGRAY           0x7BEF
#define ILI9341_COLOR_BLUE            0x001F
#define ILI9341_COLOR_BLUE2			      0x051D
#define ILI9341_COLOR_GREEN           0x07E0
#define ILI9341_COLOR_GREEN2		      0xB723
#define ILI9341_COLOR_GREEN3		      0x8000
#define ILI9341_COLOR_CYAN            0x07FF
#define ILI9341_COLOR_RED             0xF800
#define ILI9341_COLOR_MAGENTA         0xF81F
#define ILI9341_COLOR_YELLOW          0xFFE0
#define ILI9341_COLOR_WHITE           0xFFFF
#define ILI9341_COLOR_ORANGE          0xFD20
#define ILI9341_COLOR_GREENYELLOW     0xAFE5
#define ILI9341_COLOR_BROWN 			    0XBC40

typedef struct ILI9341InitStruct {
	GPIO_TypeDef * CSPort;
	GPIO_TypeDef * NWEPort;
	GPIO_TypeDef * RSPort;
	GPIO_TypeDef * ResetPort;
	uint16_t CSPin;
	uint16_t NWEPin;
	uint16_t RSPin;
	uint16_t ResetPin;
	bool MemSwap;
}ILI9341InitStruct;

class ILI9341 : public LCD {
private:
	GPIO_TypeDef * CSPort, * NWEPort, * RSPort, * ResetPort;
	uint16_t CSPin, NWEPin, RSPin, ResetPin;
	__IO uint8_t * baseAddress, * dataAddress;
	uint8_t rotation = 1;
	bool _cp437    = false; // TODO

	const unsigned char font1[1280] = {
		
	};

	void init();

	void sendCommand(uint8_t command);
	void sendData(uint8_t sendData);
	void writeRegister16(uint8_t reg, uint16_t data);
	void writeRegister32(uint8_t reg, uint32_t data);

	void drawCircleHelper(uint16_t x0, uint16_t y0, uint16_t r, uint8_t cornername, uint16_t color);
	void fillCircleHelper(uint16_t x0, uint16_t y0, uint16_t r, uint8_t cornername, uint16_t delta, uint16_t color);
protected:
	void setCursorPosition(uint16_t x0, uint16_t x1, uint16_t y0, uint16_t y1) override;
public:
	/* NOTE: constructor should not be called until after timers, DMAs, etc have been initialized/started */
	ILI9341(magna::ILI9341InitStruct& initStruct);
	~ILI9341() override {}

	void invertRows(uint16_t y0, uint16_t y1);

	void fill(uint16_t color) override;
	void drawPixel(uint16_t x, uint16_t y, uint16_t color) override;
	void drawRect(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) override;
	void fillRect(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) override;
	void drawCircle(uint16_t x0, uint16_t y0, uint16_t r, uint16_t color) override;
	void fillCircle(uint16_t x0, uint16_t y0, uint16_t r, uint16_t color) override;
	void drawTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) override;
	void fillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color) override;
	void drawChar(uint16_t x, uint16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size) override;
	void printText(std::string text, uint16_t x, uint16_t y, uint16_t color, uint16_t bg, uint8_t size) override;
	void printImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data, uint32_t size) override;
	void setRotation(uint8_t rotate) override;
	void drawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) override;

	void drawFastHLine(uint16_t x, uint16_t y, uint16_t w, uint16_t color);
	void drawFastVLine(uint16_t x, uint16_t y, uint16_t w, uint16_t color);
};

}

#endif /* INC_MAGNA_LCD_H_ */
